...
import type { TranslationMemory } from "../services/memory-repo.ts";
...
export function registerMemoryRoute(
  app: FastifyInstance,
  memory: MemoryQueryStore,
): void {
  app.get("/v1/memory", async (request) => {
    const limit = parseLimit(request.query);
    const items: readonly TranslationMemory[] = await memory.listMemory(limit);
    return { items };
  });
}