...
1. 不用账号、不用付费墙、不用默认填写 API Key，装好后就能试用。
2. 翻译接口优先选择国内网络通常能访问的服务，并带有多层自动备用。
3. 支持学术术语保护，不会把重要的专业名词直接翻译丢，点击术语还能查看知识卡。
4. 翻译过程中锁定页面布局，避免滚动到页面底部时反复抖动。

## 普通用户怎么开始？

1. 下载或构建扩展。
2. 在 Chrome 打开 `chrome://extensions/`，或者打开 Edge 的扩展管理页。
3. 开启“开发者模式”，选择“加载已解压的扩展程序”。
4. 选择项目中的 `dist/` 目录。
5. 打开英文网页，点击扩展图标或页面悬浮按钮开始翻译。

默认翻译顺序是：

`有道免费接口 → Transmart → 本地模型`

其中以前两个免密钥接口为主，本地模型负责在接口不可用或没有网络时保底。用户也可以在设置页自行添加 API Key，切换到更高精度的 AI 翻译服务。

## 这个项目与原版的区别

本项目不是闭源沉浸式翻译的反编译版本，也不包含原版代码。它是一个独立实现、MIT 许可、可以查看和修改全部源码的同类工具。

- 原版以账号、云端服务和订阅体系为中心；本项目没有账号系统、付费墙和埋点上报。
- 原版主要由官方维护翻译服务和站点规则；本项目把服务接口、站点规则、本地模型和知识库都放在可配置代码中。
- 原版默认面向全球网络；本项目额外针对简体中文用户增加了国内免费接口链、国内浏览器兼容构建和本地保底方案。
- 原版是成熟商业产品；本项目更偏向可审计、可改造、可自建的个人项目。

## 数据与隐私

- API Key 保存在浏览器的扩展本地存储中，不会写入仓库。
- 扩展不上传账号、页面历史或浏览记录。
- 翻译网页时，文本会发送到你自己选择的翻译服务。
- 本地模型下载完成后，翻译和学术辅助可以完全在本机运行。
- 仓库不提交模型权重；首次使用本地模型时才会从配置的镜像下载并缓存。
...
首次安装默认使用免密钥的国内免费翻译链：有道免费接口优先，失败后自动切换 Transmart，再回退到本地模型。目标语言默认为简体中文，不需要注册账号或填写 API Key。
...
- 阅读稳定性：翻译结果批量渲染，滚动期间锁定布局，自动适配深浅背景颜色，避免页面持续抖动。
- 悬浮控制按钮：原网页、仅中文、中英对照和学术助手四种状态；中文字号继承原文，不额外放大。
...
开启“学术助手”后，页面中识别出的学术术语会保留英文，并在译文后附加当前语境下的中文译名。学术术语使用独立的占位符保护流程，避免先被普通机器翻译覆盖。

- 单击术语：显示精简知识卡，包括译名、领域、定义和最相关的公开来源。
- 双击术语或知识卡：打开完整知识页。
- 完整知识页：左侧展示术语语境、文献来源和出现过的上下文，右侧约三分之一区域提供 AI 对话。
- 检索来源：OpenAlex、Crossref 和 Semantic Scholar，可公开访问的元数据和摘要。
- 知识缓存：术语结果保存在扩展本地存储中，默认缓存 30 天。

学术词识别会先用本地规则筛选候选词。若配置了 ChatGPT OAuth、OpenAI 兼容、Claude、Gemini 或 Azure OpenAI 服务，扩展会把这些候选词连同文章标题和上下文交给 AI 判断语义。没有配置 AI 服务时，仍会保留英文术语并使用当前翻译服务补充基础译名，但不会进行上下文推理或 AI 对话。

## 本地双模型

插件内置本地模型运行架构，模型由离屏页面下载到扩展缓存中，不需要安装 Ollama、Python 或修改系统环境。

- 翻译模型：默认使用 opus-mt-en-zh，负责英文到中文的本地翻译。
- 学术模型：默认使用 Qwen2.5-0.5B-Instruct，负责术语理解和本地 AI 对话。
- 多语种回退：其他语言自动使用 MyMemory。
- 高质量选项：WebGPU 环境可切换到 M2M100、NLLB-200 和 Qwen2.5-1.5B-Instruct。
- 模型来源：首次使用时从国内可访问的 HF Mirror 下载。
- 运行方式：默认使用单线程 WASM，兼容性优先；高级设置可切换 WebGPU。
- 回退策略：本地模型尚未完成下载时，普通翻译自动回退到 MyMemory。
- 状态控制：设置页可分别下载翻译模型和学术模型，并显示实时进度。

ONNX Runtime 及其 WASM 文件随扩展打包，不依赖运行时 CDN。模型权重不进入仓库，由扩展在首次使用时缓存。

## GitHub 发布

仓库只保存插件代码、运行时和小型配置数据，不提交模型权重。

- `main` 分支提交和 PR 自动执行类型检查、Lint、测试和构建。
- 推送 `v*` 标签后自动构建扩展并创建 GitHub Release。
- Release 附件是可解压的扩展构建，不包含大模型文件。
- 大模型通过 HF Mirror 在用户首次使用时下载。
- 真正面向普通用户的一键安装仍建议发布到 Edge Add-ons 或 Chrome Web Store。
...
设置页也可配置 Google、Bing、MyMemory、DeepL、Azure Translator、Gemini、Claude、国内机器翻译服务、OpenAI 兼容预设及自定义 HTTP 服务。默认启用的免费翻译适合直接试用；填写 MyMemory 联系邮箱可使用其更高免费额度。未填写凭据的收费服务不会自动启用。
